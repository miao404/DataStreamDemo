package org.example.stream.metrics.utils.extractor.flink;
...
public class FlinkHistogramMetricsUtil extends MetricsUtil {
    ...
    public void init(JobConfig config, String delimiter) {
        ...
        List<String> tagNames = (List)((MetricsParserConfig)config).getScopeTagsMap().get(splitedFields[1]);
        ...
        List<String> rocksdbTags = ((MetricsParserConfig)config).getRocksdbExtraTags();
        ...
        List<String> extraTagNames = (List)((MetricsParserConfig)config).getExtraTagsMap().get(metricName);
