...
import org.example.stream.metrics.utils.extractor.flink.FlinkHistogramMetricsUtil;
import org.example.stream.metrics.utils.extractor.flink.FlinkMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormMapAvgMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormMapGcMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormMapKafkaOffsetMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormMapMemoryMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormMapSumMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormSingleValueMetricsUtil;
import org.example.stream.metrics.utils.extractor.storm.StormUserDefinedMetricsUtil;
...
public class MetricsExtractor {
    ...
    private List<NormalizedMetric> metrics;
    ...
    public void init(JobConfig config) {
        ...
        List<String> tagNames = ((MetricsParserConfig)config).getOpentsdbTags();
        ...
        Map<String, List<String>> extraTagsMap = ((MetricsParserConfig)config).getExtraTagsMap();
        ...
        Map<String, List<String>> extractorType = ((MetricsParserConfig)config).getExtractorType();
        ...
        for(JsonElement metricJson : metricsArray) {
            ...
            Set<String> multiMetricsSet = ((MetricsParserConfig)config).getMultiMetricsSet();
            if (metricsUtil != null) {
                ...
                if (metricsUtil.getValid() == Boolean.TRUE) {
                    if (multiMetricsSet.contains(metricsUtil.getType())) {
                        ...
                        for(MetricsUtil.MetricsBean metric : metricsBeans) {
                            NormalizedMetric normalizedMetric = new NormalizedMetric();
                            normalizedMetric.setTs(this.getTimestamp());
                            ...
                            normalizedMetric.setTags(allTags);
                            Map<String, List<Object>> kvs = new HashMap();
                            kvs.put(metric.getName(), Arrays.asList(metric.getValue()));
                            normalizedMetric.setKvs(kvs);
                            this.getMetrics().add(normalizedMetric);
                        }
                    } else {
                        NormalizedMetric normalizedMetric = new NormalizedMetric();
                        normalizedMetric.setTs(this.getTimestamp());
    ...
    public List<NormalizedMetric> getMetrics() {
        return this.metrics;
    }

    public void setMetrics(List<NormalizedMetric> metrics) {
